...
class ImageTransformer(nn.Module):
    ...
    def sample_from_cat(self, logits, argmax=False):
        if argmax:
            ...
            import scipy as sc
            #print(logits)
            #print("entr: ", sc.stats.entropy(p.detach().cpu().numpy().flatten(), base=2), p.shape, p)
            print("entr: ", sc.stats.entropy(p.detach().cpu().numpy().flatten(), base=2))
        return sel
    ...
    def sample(self, n, device, argmax=False):
        ...
        for curr_infer_length in tqdm(range(total_len)):
            ...
            #import scipy as sc
            #print("outp: ", outputs)
            #print("entr: ", sc.stats.entropy(outputs.detach().cpu().numpy().flatten(), base=2), outputs.shape)
            #print(self.hparams.distr )
            if self.hparams.distr == "dmol":
